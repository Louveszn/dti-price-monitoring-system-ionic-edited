...
export class NuevaPage {
  ...
  loadSheet(index: number) {
    ...
    // Search for header row in entire sheet, not just first 10 rows
    let headerRowIndex = -1;
    for (let i = 0; i < json.length; i++) {
      if (
        json[i].some(c =>
          c?.toString().toLowerCase().includes('prevailing price')
        )
      ) {
        headerRowIndex = i;
        break;
    ...
    // If no header row found, default to first row
    if (headerRowIndex === -1) {
      headerRowIndex = 0;
    }
  ...
  detectColumnIndexes() {
    // Find current month price column - search through ALL headers
    this.currentIndex = -1;
    for (let i = 0; i < this.fullTableHeaders.length; i++) {
      const header = this.fullTableHeaders[i]?.toLowerCase() || '';
      if (
        header.includes('prevailing price') &&
        header.includes('for the month') &&
        !header.includes('month ago')
      ) {
        this.currentIndex = i;
        break;
      }
    ...
    // Find 1 month ago column - search through ALL headers
    this.month1Index = -1;
    for (let i = 0; i < this.fullTableHeaders.length; i++) {
      const header = this.fullTableHeaders[i]?.toLowerCase() || '';
      if (header.includes('1 month ago') || header.includes('1-month ago')) {
        this.month1Index = i;
        break;
      }
    }

    // Find 3 months ago column - search through ALL headers
    this.month3Index = -1;
    for (let i = 0; i < this.fullTableHeaders.length; i++) {
      const header = this.fullTableHeaders[i]?.toLowerCase() || '';
      if (header.includes('3 months ago') || header.includes('3-months ago')) {
        this.month3Index = i;
        break;
      }
    }

    // Find product name column - search through ALL headers
    ...
    // Fourth try: look for columns with unit keywords in header
    if (this.unitIndex === -1 && this.productNameIndex !== -1 && this.currentIndex !== -1) {
      for (let i = this.productNameIndex + 1; i < this.currentIndex; i++) {
        const header = this.fullTableHeaders[i]?.toString().trim() || '';
        
        if (header.length > 0 && header.length < 10) {
          const lowerHeader = header.toLowerCase();
          if (
            lowerHeader.includes('kg') ||
            lowerHeader.includes('pc') ||
            lowerHeader.includes('liter') ||
            lowerHeader.includes('pack') ||
            this.looksLikeUnitColumn(i)
          ) {
            this.unitIndex = i;
            break;
          }
    ...
    // Fifth try: assume column right after product name
    if (this.unitIndex === -1 && this.productNameIndex !== -1) {
      ...
      if (potentialUnitIndex < this.fullTableHeaders.length) {
        const header = this.fullTableHeaders[potentialUnitIndex]?.toString() || '';
        if (header.length < 15 || this.looksLikeUnitColumn(potentialUnitIndex)) {
  ...
  onSearchChange() {
    ...
    const filtered = this.fullTableData.filter(row => {
      return row.some(cell => {
  ...
  calculateMonthSummary(compareIndex: number): MonthSummary {
    ...
    for (const row of this.fullTableData) {
      ...
      if (productName && productName.toString().trim() !== '') {
        ...
        const peso1Price = this.peso1Index !== -1 ? this.parseNumber(row[this.peso1Index]) : null;
        const peso3Price = this.peso3Index !== -1 ? this.parseNumber(row[this.peso3Index]) : null;
  ...
  getComparisonClass(rowIndex: number, colIndex: number): string {
    ...
    const originalRowIndex = this.fullTableData.findIndex(row => {
      if (!this.isComparativeMode) return false;
      
      return this.visibleColumns.every((colIdx, i) => {
        return row[colIdx] === currentRow[i];
      });
    });
